...
class ViewBooking extends Component
{
    ...
    public function acceptIncentive($id)
    {
        ...
        if ($booking->date_completed != null) {
            $sixDaysAfterTripEnd = Carbon::parse($booking->trip_end_date)->addDays(6);
            if ($sixDaysAfterTripEnd >= Carbon::now() && $sixDaysAfterTripEnd >= Carbon::parse($booking->trip_end_date)) {
                $incentive->update(['is_accept' => Cash::STATUS_ACCEPTED]);
                $this->dispatchBrowserEvent('show-success', ['message' => 'Incentive accepted successfully']);
            } else {
                /*declinme*/
//                $incentive->update(['is_accept' => Cash::STATUS_DECLINED]);
                $this->dispatchBrowserEvent('show-error', ['message' => 'Incentive cannot be accepted if they are not within 6 days of the end date of the trip.']);
            }
        }
        else {
    ...
    public function acceptExpense($id)
    {
        ...
        if ($booking->date_completed != null) {
            ...
            if ($sixDaysAfterTripEnd >= Carbon::now() && $sixDaysAfterTripEnd >= Carbon::parse($booking->trip_end_date)) {
                ...
                $this->dispatchBrowserEvent('show-success', ['message' => 'Expense accepted successfully']);
            } else {
                /*declinme*/
//                $expense->update(['is_accept' => Cash::STATUS_DECLINED]);
                $this->dispatchBrowserEvent('show-error', ['message' => 'Expense cannot be accepted if they are not within 6 days of the end date of the trip.']);
            }
        }
        else {
